...
public class SlimeIA : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!enemyStopped && _gameManager != null && _gameManager.isGameOver)
        {
            enemyStopped = true;
            StopEnemy();
        }
    ...
    {
        if (isDead || _gameManager.isGameOver) return;
    ...
    private IEnumerator FURY()
    {
        ...
        while(!isDead && !_gameManager.isGameOver)
        {
    ...
    public void Attack()
    {
        if(isAttack || isDead || _gameManager.isGameOver    ) return;
